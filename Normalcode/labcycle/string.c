...
#define Name_len 50
int i , j;
void insert (char names[][Name_len] , int *count ) {
    if (*count < MAX) {
...
void delete (char names[][Name_len] , int *count) {
...
void ToSort (char names[][Name_len] , int count) {
    if (count > 1) {
        char temp[Name_len];
        for (i = 0 ; i < count - 1 ; i++){
            for ( j = i+ 1; j < count ; j++) {
...
void printList (char names[][Name_len] , int count ) {
    if (count > 0) {
        for (i = 0 ; i < count ; i++) {
        printf ("%s\n", names[i]);
    }
